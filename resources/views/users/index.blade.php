...
@section('navbar-content')
        ...
        <section class="bg-gray-50 dark:bg-gray-900 p-3 sm:p-5">
            ...
            <div class="mx-auto max-w-screen-xl px-4 lg:px-12">
                ...
                <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden">
                            ...
                            <form action = "{{ route('users.index') }}" method="GET" class="flex items-center">
                                ...
                                <div class="relative w-full">
                                    ...
                                    <input type="text" id="simple-search" name ="search"
                                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-2/3 pl-10 p-2 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                        placeholder="Search" value="{{ request()->query('search') }}">
                                </div>
                                <button type="submit"
                                    class="hidden mt-4 w-full shrink-0 rounded-lg bg-blue-700 px-5 py-2.5 text-sm font-medium text-white hover:bg-blue-800 focus:outline-none focus:ring-4 focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800 sm:mt-0 sm:w-auto">Search</button>
                            </form>
                        </div>
                        <div
                        ...
                        <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                            <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                                <tr>
                                    ...
                                    <th scope="col" class="px-4 py-3 w-[300px]">Name</th>
                                    <th scope="col" class="px-4 py-3 w-[200px]">Designation</th>
                                    <th scope="col" class="px-4 py-3 w-[300px]">Department</th>
                                    <th scope="col" class="px-4 py-3 w-[100px]">Active</th>
                            ...
                            <tbody class="min-h-[200px]">
                                @foreach ($users as $userm)
                                    <tr class="border-b dark:border-gray-700">
                                        <td class="px-4 py-3 w-[100px]">{{ $userm->empid }}</td>
                                        <td class="px-4 py-3 w-[300px] flex items-center space-x-3">
                                            @php
                                                $userColor = '#' . substr(md5($userm->id), 0, 6);
                                            @endphp
                                            ...
                                            @else
                                                <span
                                                    ...
                                                    {{ strtoupper(substr($userm->fname, 0, 1)) }}{{ strtoupper(substr($userm->lname, 0, 1)) }}
                                                </span>
                                            @endif
                                        ...
                                        <td class="px-4 py-3 w-[120px] text-center">
                                            <div class="relative inline-block">
                                                <button id="dropdown-btn-{{ $userm->id }}"
                                                    data-dropdown-toggle="dropdown-menu-{{ $userm->id }}"
                                                ...
                                                <div id="dropdown-menu-{{ $userm->id }}"
                                                    ...
                                                    <ul class="py-1 text-sm text-gray-700 dark:text-gray-200">
                                                        <li>
                                                            <a href="{{ route('users.edit', $userm->id) }}"
                    ...
                    <!-- Pagination Links -->
                    <div
                        class="w-full md:w-auto flex flex-col md:flex-row space-y-2 md:space-y-0 items-stretch md:items-center justify-end md:space-x-3 flex-shrink-0 mb-4 mt-4 mr-4">
                        {{ $users->links() }}
                    </div>
            ...
            document.querySelectorAll('.toggle-status').forEach(toggle => {
                toggle.addEventListener('change', function() {
                    ...
                    fetch(`/users/toggle-status/${userId}`, {
                        ...
                        .then(data => {
                            if (data.success) {
                                this.closest('label').querySelector('.status-text').innerText = data.status;
                            } else {
    ...
    @else
        <p>Please <a href="{{ route('login') }}">login</a> to access users.</p>
    @endif
@endsection

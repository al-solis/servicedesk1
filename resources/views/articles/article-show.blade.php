            ...
            <form action="#">
                <div>
                    <span
                        ...
                        {{ $articles->category->description }} </span>
                    ...
                    <div class="flex items-center text-sm font-medium text-gray-500 dark:text-gray-400 mb-4">
                        ...
                        @if ($articles->user->profile_picture)
                            <img class="w-10 h-10 border-2 border-white rounded-full dark:border-gray-800"
                                {{-- src="{{ Storage::url($articles->user->profile_picture) }}"  --}} src="{{ asset('storage/' . $articles->user->profile_picture) }}"
                        ...
                        @endif
                        &nbsp; Added by {{ $articles->user->lname }}, {{ $articles->user->fname }}
                        {{ substr($articles->user->mname, 0, 1) }}. on &nbsp; <time
                            datetime="2021-01-06">{{ $articles->created_at->format('M d, Y') }}
                            {{ $articles->created_at->format('h:i:sa') }}</time>

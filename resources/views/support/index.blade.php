        ...
        <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden">
            <div class="flex flex-col md:flex-row items-center justify-between space-y-3 md:space-y-0 md:space-x-4 p-4">
                <div class="w-full md:w-1/2">
                    <form action="{{ route('support.index')}}" method="GET" class="flex items-center">
                        ...
                        <button type="button" class="hidden mt-4 w-full shrink-0 rounded-lg bg-blue-700 px-5 py-2.5 text-sm font-medium text-white hover:bg-blue-800 focus:outline-none focus:ring-4 focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800 sm:mt-0 sm:w-auto">Search</button>
                    </form>
                </div>
            ...
            {{-- <div class="overflow-x-auto" style="min-height: 500px;">
                <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                        <tr>
                            <th scope="col" class="px-4 py-3 w-[200px]">ID</th>
                            <th scope="col" class="px-4 py-3 w-[400px]">Team Name</th>
                            <th scope="col" class="px-4 py-3 w-[100px]">Members</th>
                ...
                <tbody class="min-h-[200px]">
                    ...
                    <tr class="border-b dark:border-gray-700">
                        <td class="px-4 py-3 w-[200px]">{{ $team->id }}</td>                        
                        <td class="px-4 py-3 w-[400px]">{{ $team->name }}</td>
                        <td class="px-4 py-3 w-[100px] text-center">
                            <div class="flex -space-x-4 rtl:space-x-reverse">
                                @foreach($supportTeam as $team)
                                <div class="flex items-center justify-between p-4 bg-white border rounded-lg shadow-md dark:bg-gray-800 dark:border-gray-700">
                                    <div>
                                        ...
                                        <div class="flex -space-x-4 rtl:space-x-reverse mt-2">
                                            @foreach($team->supportMembers->take(4) as $member)
                                            ...
                                            @if($team->supportMembers->count() > 4)
                                    ...
                                    <a href="{{ route('support.show', $team->id) }}" class="text-blue-600 dark:text-blue-400 hover:underline">
                                        View Details
                                    </a>
                                </div>
                            @endforeach
                        ...
                        <td class="px-4 py-3 w-[120px] text-center">
                            <div class="relative inline-block">
                                <button id="dropdown-btn-{{ $team->id }}" data-dropdown-toggle="dropdown-menu-{{ $team->id }}" 
                                ...
                                <div id="dropdown-menu-{{ $team->id }}" 
                                    ...
                                    <ul class="py-1 text-sm text-gray-700 dark:text-gray-200">
                                        <li>
                                            <a href="{{ route('support.show', $team->id) }}" class="block py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">Show</a>
                                        </li>
                                        <li>
                                            <a href="{{ route('support.edit', $team->id) }}" class="block py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">Edit</a>
                                        </li>
                                    </ul>                                    
                                </div>
                            </div>                    
            ...
            <div class="overflow-x-auto" style="min-height: 500px;">
                @foreach($supportTeam as $team)
                    ...
                    <div class="flex items-center justify-between p-3 bg-white border rounded-lg shadow-md dark:bg-gray-800 dark:border-gray-700">
                        <div>
                            <h3 class="text-lg font-semibold text-gray-900 dark:text-white">{{ $team->name }}</h3>
                            <div class="flex -space-x-6 rtl:space-x-reverse">
                                
                                @if (!empty($team->supportMembers) && $team->supportMembers->count() > 0)
                                    @foreach($team->supportMembers->take(4) as $member)   
                                    <div class="relative group">
                                        @if ($member->user)        
                                            @php
                                                $userColor = '#' . substr(md5($member->user->id), 0, 6);
                                            @endphp
                        ...
                        {{-- <a href="{{ route('support.edit', $team->id) }}" class="text-blue-600 dark:text-blue-400 hover:underline text-sm">
                            Update
                        </a> --}}
                    </div>

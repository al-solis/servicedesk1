...
<section class="bg-white py-8 antialiased dark:bg-gray-900 md:py-16 ml-0 md:ml-16 lg:ml-32">
    <div class="mx-auto max-w-screen-lg px-2 2xl:px-2 mt-8">
      <div class="lg:flex lg:items-center lg:justify-between lg:gap-4">
        <h2 class="shrink-0 text-xl font-semibold text-gray-900 dark:text-white sm:text-2xl">Knowledge Base Article ({{count($articles)}})</h2>
  
        <form action="{{ route('articles.article-index') }}" method="GET" class="mt-4 w-full gap-4 sm:flex sm:items-center sm:justify-end lg:mt-0">
          ...
          <div class="relative w-full flex-1 lg:max-w-sm">
            ...
            <input type="text" id="simple-search" name="search" class="block w-full rounded-lg border border-gray-300 bg-gray-50 px-4 py-2.5 ps-9 text-sm text-gray-900 focus:border-blue-500 focus:ring-blue-500 dark:border-gray-600 dark:bg-gray-700 dark:text-white dark:placeholder:text-gray-400 dark:focus:border-blue-500 dark:focus:ring-blue-500" placeholder="Search" 
            value="{{ request()->query('search') }}">
          </div>
  
          <button type="button" class="hidden mt-4 w-full shrink-0 rounded-lg bg-blue-700 px-5 py-2.5 text-sm font-medium text-white hover:bg-blue-800 focus:outline-none focus:ring-4 focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800 sm:mt-0 sm:w-auto">Ask a question</button>
        </form>
      </div>
        ...
        @foreach ($articles as $article)         
            ...
            <div class="mt-6 flow-root">
                ...
                <div class="space-y-4 py-6 md:py-8">
                    ...
                    <div>
                        <span class="inline-block rounded bg-green-100 px-2.5 py-0.5 text-xs font-medium text-green-800 dark:bg-green-900 dark:text-green-300 md:mb-0"> {{ $article->category->description}} </span>
                    </div>        
                    ...
                    <div class="flex items-center justify-between text-sm font-medium text-gray-500 dark:text-gray-400">
                      <div class="flex items-center space-x-2">
                          ...
                          <span>
                              Added by {{ $article->user->lname }}, {{ $article->user->fname }} {{ substr($article->user->mname, 0, 1) }}. 
                              on <time datetime="{{ $article->created_at }}">{{ $article->created_at->format('M d, Y h:i A') }}</time>
                          </span>
                      </div>
                  
                      <a href="{{ route('articles.article-show', $article->id)}}" class="text-blue-600 dark:text-blue-400 hover:underline">
                          Read more
                      </a>
                  </div>

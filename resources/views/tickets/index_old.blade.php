...
<div class="container">
    ...
    <button class="btn btn-primary" onclick="openTicketModal('{{ route('tickets.create') }}')">New Ticket</button>
    <table class="table table-bordered mt-3">
        ...
        <tbody>
            ...
            <tr>
                <td style="width:100px; text-align: left;">{{ $ticket->id }}</td>
                <td style="width: 200px; text-align: left; align-items: center;">
                    @php
                        // Generate a consistent random color based on the user's ID
                        $userColor = '#' . substr(md5($ticket->user->id), 0, 6);
                    @endphp
                    <span style="width: 40px; height: 40px; border-radius: 50%; display: inline-flex; justify-content: center; align-items: center; background-color: {{ $userColor }}; color: white; font-size: 16px; font-weight: bold; margin-right: 10px;">
                        {{ strtoupper(substr($ticket->user->fname, 0, 1)) }}
                    </span>
                    {{ $ticket->user->lname }}, {{ $ticket->user->fname }} {{ substr($ticket->user->mname, 0, 1) }}.
                </td>                
                <td>{{ $ticket->description }}</td>
                <td style="width:120px; text-align: center;">
                    <span class="badge"
                        style="
                        @if($ticket->status == 'Open') background-color: #cce5ff; color: #004085; 
                        @elseif($ticket->status == 'In Progress') background-color: #d6d8d9; color: #383d41; 
                        @elseif($ticket->status == 'Closed') background-color: #d4edda; color: #155724; 
                        @elseif($ticket->status == 'On-hold') background-color: #e2d9f3; color: #4a148c; 
                        @elseif($ticket->status == 'Cancelled') background-color: #f8d7da; color: #721c24; 
                        @endif
                    ...
                    {{-- <span class="badge 
                        ...
                        @elseif($ticket->status == 'On-hold') bg-purple
                ...
                <td style="width: 120px; text-align: center">
                    @if(Auth::user()->usertype == 'User')
                        <button class="btn btn-warning" onclick="openTicketModal('{{ route('tickets.show', $ticket->id) }}')">View</button>
                    @else
                        <button class="btn btn-warning" onclick="openTicketModal('{{ route('tickets.edit', $ticket->id) }}')">Update</button>
                    @endif
                </td>
            </tr>
            @endforeach            
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <!-- Dynamic content area (if needed) -->
                <div id="dynamicContent">
                    <!-- Content will be loaded here dynamically if needed -->
                </div>                
...
function openTicketModal(url) {
    fetch(url)
        ...
        .then(html => {
            document.getElementById('dynamicContent').innerHTML = html; // Use a different ID
